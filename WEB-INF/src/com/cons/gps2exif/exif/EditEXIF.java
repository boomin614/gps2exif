...
public class EditEXIF {
   ...
   * @throws ImageWriteException
   */
  public void setExifGPSTag(final File jpegImageFile, final File outputDir, double longitude, double latitude) throws IOException, ImageReadException, ImageWriteException {
    OutputStream os = null;
    boolean canThrow = false;
    ...
    try {
      ...
      if (null != jpegMetadata) {
        ...
        // 出力ディレクトリと出力先ファイル名の設定
        if (null != outputDir | !outputDir.exists()) {
          outputFilePath = outputDir + "/" + jpegImageFile.getName();
        } else {
          throw new IOException("Not output directory is set yet: " + outputDir);
        }
        ...
        if (null != exif) {
          ...
          // TiffOutputSet class represents the Exif data to write.
          //
          // Usually, we want to update existing Exif metadata by changing the values of a few
          // fields, or adding a field.
          // In these cases, it is easiest to use getOutputSet() to start with a "copy" of the
